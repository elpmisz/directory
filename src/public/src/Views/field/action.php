...
if ($action === "create") {
  try {
    ...
    $count = $FIELD->field_count([$name]);
    if (intval($count) === 0 && !empty($name)) {
      $FIELD->field_insert([$name]);
    }
...
if ($action === "update") {
  try {
    ...
    $FIELD->field_update([$name, $status, $uuid]);

    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/field/edit/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
