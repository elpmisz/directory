...
if ($action === "create") {
  try {
    ...
    $count = $GROUP->group_count([$name]);
    if (intval($count) === 0 && !empty($name)) {
      $GROUP->group_insert([$name]);
    }
...
if ($action === "update") {
  try {
    ...
    $GROUP->group_update([$name, $status, $uuid]);

    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/group/edit/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
