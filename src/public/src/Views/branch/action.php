...
if ($action === "create") {
  try {
    ...
    $count = $BRANCH->branch_count([$name]);
    if (intval($count) === 0 && !empty($name)) {
      $BRANCH->branch_insert([$name]);
    }
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/branch");
...
if ($action === "update") {
  try {
    ...
    $BRANCH->branch_update([$name, $status, $uuid]);

    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/branch/edit/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
