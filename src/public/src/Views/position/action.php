...
if ($action === "create") {
  try {
    ...
    $count = $POSITION->position_count([$name]);
    if (intval($count) === 0 && !empty($name)) {
      $POSITION->position_insert([$name]);
    }
...
if ($action === "update") {
  try {
    ...
    $POSITION->position_update([$name, $status, $uuid]);

    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/position/edit/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
