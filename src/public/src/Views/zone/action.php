...
if ($action === "create") {
  try {
    ...
    $count = $ZONE->zone_count([$name]);
    if (intval($count) === 0 && !empty($name)) {
      $ZONE->zone_insert([$name]);
    }
...
if ($action === "update") {
  try {
    ...
    $ZONE->zone_update([$name, $status, $uuid]);

    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/zone/edit/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
