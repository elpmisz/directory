...
class Group
{
  ...
  public function group_view($data)
  {
    $sql = "SELECT * FROM directory.group WHERE uuid = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    return $stmt->fetch();
  }
  ...
  {
    $sql = "UPDATE directory.group SET 
    name = ?,
    status = ?,
    updated = NOW()
    WHERE uuid = ?";
  ...
  {
    $sql = "SELECT COUNT(*) FROM directory.group";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    ...
    foreach ($result as $row) {
      $action = "<a href='/group/edit/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a> <a href='javascript:void(0)' class='badge badge-danger font-weight-light btn-delete' id='{$row['uuid']}'>ลบ</a>";
