...
class Position
{
  ...
  {
    $sql = "SELECT * FROM directory.position WHERE uuid = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    return $stmt->fetch();
  }
  ...
  {
    $sql = "UPDATE directory.position SET 
    name = ?,
    status = ?,
    updated = NOW()
    WHERE uuid = ?";
  ...
  public function position_delete($data)
  {
    $sql = "UPDATE directory.position SET 
    status = 0,
    updated = NOW()
  ...
  {
    $sql = "SELECT COUNT(*) FROM directory.position";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    $total = $stmt->fetchColumn();

    $column = ["a.id", "a.email", "a.position", "a.department", "a.area", "a.field", "a.position", "b.text", "c.text"];
    ...
    foreach ($result as $row) {
      $action = "<a href='/position/edit/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a> <a href='javascript:void(0)' class='badge badge-danger font-weight-light btn-delete' id='{$row['uuid']}'>ลบ</a>";
